...
class CableCalView(FormView):
    ...
    def form_valid(self, form):
        ...
        all_reels = ReelsList.objects.values_list('name', 'height', 'width', 'diameter', 'diameter_neck', 'length_neck', 'reel_type', 'mass', 'max_load')
        if all_reels:
            r_names, r_heights, r_widths, r_diameters, r_diameter_necks, r_length_necks, r_reel_types, r_masses, r_max_loads = zip(*all_reels)
        else:
            r_names, r_heights, r_widths, r_diameters, r_diameter_necks, r_length_necks, r_reel_types, r_masses, r_max_loads = ([], [], [], [], [], [], [], [], [])
        ...
        for c in range(len(diameters)):
            ...
            for r in range(len(r_names)):
                L = math.pi * r_length_necks[r] * ((r_diameters[r] - 100) ** 2 - r_diameter_necks[r] ** 2) * 0.93 / (
        ...
        brutto_1 = [round(float(a) * float(b) + float(c)) for a, b, c in zip(reel_len, masses, r_masses)]
        netto_all = [round(float(a) * float(b)) for a, b in zip(order_lens, masses)]
        brutto_all = [round(float(a) * float(b) + float(c) * float(d)) for a, b, c, d in zip(order_lens, masses, r_masses, reel_num)]
